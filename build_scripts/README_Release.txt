...
Step 2: 在網頁介面設定關鍵參數
  ...
  - nolworld：NOL 自動登入、預售、排隊、10 分鐘重啟、鎖票、票池、區塊掃描與安全驗證交接
  - 選擇 NOL World 時不使用通用自動刷新間隔；建議 NOL 監控 5 秒、區塊掃描 600 ms
  - NOL 活動頁會自動關閉 Notice，等待遮罩消失後點擊 Buy Now（包含首次啟動）
  - NOL 日期、時間與 Next 會分步確認；官方互動式安全驗證會保留狀態，完成後自動續跑
...
Step 4: 監控執行
  ...
  - 官方 Turnstile／互動式驗證及付款由使用者依網站流程完成；程式會保留狀態並自動續跑
