...
from platforms.nolworld import *
...
async def nodriver_goto_homepage(driver, config_dict):
    ...
    if any(domain in homepage for domain in (
        'world.nol.com',
        'nol.com',
        'interpark.com',
        'globalinterpark.com',
    )):
        nol_account = config_dict.get("accounts", {}).get(
            "nolworld_account",
            "",
        ).strip()
        nol_password = config_dict.get("accounts", {}).get(
            "nolworld_password",
            "",
        )
        if (
            ...
            homepage = build_nolworld_login_url(homepage)
...
async def main(args):
    ...
    url = ""
    last_url = ""
    security_handoff = HandoffCoordinator(notice_interval=15)
    security_last_check_at = 0.0
    security_last_url = ""
    runtime_events = StructuredLogger(
        "runtime",
        enabled=config_dict.get("advanced", {}).get("verbose", False),
    )
    ...
    while True:
        ...
        platform_key = resolve_platform(url)

        # Full-page protected verification uses a non-destructive handoff.
        # Embedded login widgets are left to the platform adapter so credentials
        # can still be filled before the user completes the official challenge.
        lowered_url = url.lower()
        embedded_login = (
            "cityline.com/login.html" in lowered_url
            or (
                platform_key == "nolworld"
                and (
                    "/login" in lowered_url
                    or "/auth-web/login" in lowered_url
                )
            )
        )
        security_now = time.monotonic()
        security_check_due = (
            security_handoff.active
            or url != security_last_url
            or security_now - security_last_check_at >= 3.0
        )
        if security_check_due and not embedded_login:
            security_last_check_at = security_now
            security_last_url = url
            challenge_active = await detect_cloudflare_challenge(
                tab,
                show_debug=config_dict.get("advanced", {}).get(
                    "verbose",
                    False,
                ),
            )
            handoff_event = security_handoff.observe(
                active=challenge_active,
                kind="cloudflare_turnstile",
                url=url,
                now=security_now,
            )
            if handoff_event.action in {"entered", "reminder"}:
                print(
                    "[SECURITY HANDOFF] Official verification is waiting. "
                    "The page and workflow state are preserved."
                )
                runtime_events.emit(
                    "security_handoff",
                    action=handoff_event.action,
                    kind=handoff_event.kind,
                    platform=platform_key,
                    url=url,
                )
            elif handoff_event.action == "resumed":
                print(
                    "[SECURITY HANDOFF] Verification completed; "
                    "automation resumed."
                )
                runtime_events.emit(
                    "security_resume",
                    kind=handoff_event.kind,
                    platform=platform_key,
                    elapsed_seconds=handoff_event.elapsed_seconds,
                )
            if challenge_active:
        ...
        try:
            if platform_key == "kktix":
                is_quit_bot = await nodriver_kktix_main(
                    tab,
                    url,
                    config_dict,
                )
                # Keep each multi-instance worker independent.
                if is_quit_bot:
                    is_quit_bot = False
                ...
                and not ticketplus_purchase_done
            ):
                tp_status = await nodriver_ticketplus_main(
                    tab,
                    url,
                    config_dict,
                    ocr,
                ...
                if isinstance(tp_status, dict):
                    if tp_status.get("purchase_completed", False):
                        ...
                        tp_status.get("is_ticket_assigned", False)
                        and "/confirm/" in url.lower()
                    ):
                        print(
                            "[SUCCESS] TicketPlus on confirmation page, "
                            "booking successful"
                        )
                        ticketplus_purchase_done = True
            ...
            elif platform_key == "nolworld":
                await nodriver_nolworld_main(
                    tab,
                    url,
                    config_dict,
                    ocr,
                ...
                if FANSIGO_COGNITO_DOMAIN in url:
                    ...
                    tab = await nodriver_fansigo_main(
                        tab,
                        url,
                        config_dict,
                    )
            ...
            latency_ms = round((time.monotonic() - dispatch_started) * 1000, 1)
            runtime_events.emit(
                "platform_error",
                platform=platform_key,
                url=url,
            ...
            util.create_debug_logger(config_dict).log(
                f"[PLATFORM][{platform_key or 'unknown'}] "
                f"dispatch failed after {latency_ms} ms: {exc}"
            )
            await asyncio.sleep(0.5)
        else:
            runtime_events.emit(
