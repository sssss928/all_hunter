                    ...
                    try:
                        ...
                        if modal_content and isinstance(modal_content, dict) and modal_content.get('found'):
                            ...
                            if dismiss_result is True:
                                ...
                                dismiss_success = dismiss_result[0] is True
...
async def _nodriver_tixcraft_submit_captcha_if_ready(tab, config_dict, source="manual"):
    """Submit after captcha, ticket quantity and agreement are ready."""
    debug = util.create_debug_logger(config_dict)
    ...
    try:
        ...
        debug.log(f"[TIXCRAFT CAPTCHA] Agreement check before submit failed: {exc}")
    ...
    result = await tab.evaluate(f'''
        (function() {{
            ...
            const captchaValue = (verify?.value || '').trim();
            const captchaOk = captchaValue.length === 4;
            ...
            const targetText = {ticket_number_js};
            ...
            )).filter(select => select && !select.disabled && visible(select));
            const selectMatches = select => {{
            ...
            let matched = selects.find(selectMatches);
            if (!matched && selects.length > 0) {{
                for (const select of selects) {{
                    let option = Array.from(select.options).find(item =>
                        item.value === targetText && !item.disabled
                    );
                    if (!option && allowLess) {{
                        option = Array.from(select.options)
                            .filter(item => {{
                                const value = parseInt(item.value, 10);
                                return !item.disabled && value > 0 && value <= target;
                            }})
            ...
            const ticketOk = isTicketmaster || selects.length === 0 || Boolean(matched);
            const ready = Boolean(verify) && captchaOk && agreeOk && ticketOk;
            if (!ready) {{
                return {{
                    submitted: false,
                    reason: 'not_ready',
            ...
            if (form && typeof form.requestSubmit === 'function') {{
                form.requestSubmit();
                return {{submitted: true, method: 'requestSubmit', source: {source_js}}};
            }}
            verify.dispatchEvent(new KeyboardEvent('keydown', {{
                key: 'Enter',
                code: 'Enter',
                bubbles: true
            }}));
            verify.dispatchEvent(new KeyboardEvent('keyup', {{
                key: 'Enter',
                code: 'Enter',
                bubbles: true
            }}));
            return {{submitted: true, method: 'enter', source: {source_js}}};
        }})();
    ''')
    result = util.parse_nodriver_result(result)
    ...
    if result.get("submitted", False):
        _state["captcha_submit_until"] = time.time() + 2.5
        debug.log(
            "[TIXCRAFT CAPTCHA] Submitted after captcha ready "
            f"source={source} method={result.get('method')}"
        )
        return True

    debug.log(
        "[TIXCRAFT CAPTCHA] Submit skipped - "
        f"Ticket:{result.get('ticket')} Select:{result.get('ticket_select')} "
        f"Captcha:{result.get('captcha')} Length:{result.get('captcha_length')} "
        f"Agreement:{result.get('agree')}"
    )
    return False
...
async def nodriver_tixcraft_keyin_captcha_code(tab, answer="", auto_submit=False, config_dict=None):
    ...
    if form_verifyCode:
        ...
        if is_visible:
            ...
            if inputed_value and not answer:
                normalized_value = inputed_value.strip()
...
async def nodriver_tixcraft_main(tab, url, config_dict, ocr, Captcha_Browser):
    ...
    if '/ticket/checkout' in url:
        if _state["start_time"] is not None:
            if _state["done_time"] is not None:
