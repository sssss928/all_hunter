...
const nol_check_interval = document.querySelector('#nol_check_interval');
const nol_pre_sale_mode = document.querySelector('#nol_pre_sale_mode');
const nol_lock_only_mode = document.querySelector('#nol_lock_only_mode');
const nol_auto_notice_and_buy = document.querySelector('#nol_auto_notice_and_buy');
const nol_block_delay_ms = document.querySelector('#nol_block_delay_ms');
const nol_custom_blocks = document.querySelector('#nol_custom_blocks');
const nol_ticket_pool_enabled = document.querySelector('#nol_ticket_pool_enabled');
...
<div class="alert alert-info" role="alert">
  <p class="mb-0"><strong>Version</strong>: Tickets Hunter (2026.06.23) | <strong>Technical support</strong>: Claude Code AI-assisted development</p>
</div>
...
<div class="mb-0">
  ...
  <button type="button" class="btn btn-outline-secondary btn-sm me-2 mb-2" onclick="fillHomepage('https://world.nol.com')">NOL World / Interpark</button>
</div>`);
    ...
    setRowLabelForField('nol_check_interval', fieldLabel('NOL check interval (sec)', 'nol_check_interval'));
    setRowLabelForField('nol_pre_sale_mode', fieldLabel('NOL pre-sale mode', 'nol_pre_sale_mode'));
    setRowLabelForField('nol_lock_only_mode', fieldLabel('NOL lock-only mode', 'nol_lock_only_mode'));
    setRowLabelForField('nol_auto_notice_and_buy', fieldLabel('Automatic Notice / Buy', 'nol_auto_notice_and_buy'));
    setSwitchLabelForInput('nol_auto_notice_and_buy', 'Close Notice and click Buy Now automatically, including on first launch');
    setRowLabelForField('nol_block_delay_ms', fieldLabel('NOL block scan delay (ms)', 'nol_block_delay_ms'));
    setRowLabelForField('nol_custom_blocks', fieldLabel('NOL custom seat blocks', 'nol_custom_blocks'));
    setRowLabelForField('nol_ticket_pool_enabled', fieldLabel('NOL ticket-pool query', 'nol_ticket_pool_enabled'));
    setRowLabelForField('nol_ticket_pool_refresh_minutes', fieldLabel('Ticket-pool refresh (min)', 'nol_ticket_pool_refresh_minutes'));
    setRowLabelForField('nol_security_handoff', fieldLabel('NOL security handoff', 'nol_security_handoff'));
    setSwitchLabelForInput('nol_security_handoff', 'Preserve workflow state and resume after protected verification is legitimately completed');
    setRowLabelForField('nol_session_timeout_minutes', fieldLabel('NOL booking timeout (min)', 'nol_session_timeout_minutes'));
...
function renderAutofillTabTranslations() {
    ...
    setInputGroupTexts('nolworld_account', ['Email', 'Password']);
    ...
    setRowLabelForField('nolworld_account', fieldLabel('NOL World', 'nolworld_account'));
...
const PLATFORM_MAP = [
    ...
    { key: 'nolworld',    domains: ['world.nol.com', 'nol.com', 'interpark.com', 'globalinterpark.com'] },
];
...
function updateAutoReloadVisibility(platform) {
    const row = document.getElementById('auto-reload-page-interval-row');
    if (!row) return;
    row.classList.toggle('disappear', platform === 'nolworld');
}
...
function load_settins_to_form(settings)
{
    if (settings)
    {
        ...
        nol_check_interval.value = nolworld.check_interval ?? 5;
        nol_pre_sale_mode.checked = nolworld.pre_sale_mode ?? false;
        nol_lock_only_mode.checked = nolworld.lock_only_mode ?? false;
        nol_auto_notice_and_buy.checked = nolworld.auto_notice_and_buy ?? true;
        nol_block_delay_ms.value = nolworld.block_delay_ms ?? 600;
        nol_custom_blocks.value = nolworld.custom_blocks ?? '';
        nol_ticket_pool_enabled.checked = nolworld.ticket_pool_enabled ?? true;
...
function save_changes_to_dict(silent_flag)
{
    ...
    if (!ticket_number_value)
    {
        ...
        if(settings) {
            ...
            if (!settings.nolworld) settings.nolworld = {};
            settings.nolworld.check_interval = Math.max(3, Math.min(120, Number(nol_check_interval.value) || 5));
            settings.nolworld.pre_sale_mode = nol_pre_sale_mode.checked;
            settings.nolworld.lock_only_mode = nol_lock_only_mode.checked;
            settings.nolworld.auto_notice_and_buy = nol_auto_notice_and_buy.checked;
            settings.nolworld.block_delay_ms = Math.max(200, Math.min(5000, parseInt(nol_block_delay_ms.value) || 600));
            settings.nolworld.custom_blocks = nol_custom_blocks.value.trim();
            settings.nolworld.ticket_pool_enabled = nol_ticket_pool_enabled.checked;
...
function check_unsaved_fields()
{
    if(settings) {
        ...
        const nolworldFields = {
            nol_check_interval: settings.nolworld?.check_interval ?? 5,
            nol_pre_sale_mode: settings.nolworld?.pre_sale_mode ?? false,
            nol_lock_only_mode: settings.nolworld?.lock_only_mode ?? false,
            nol_auto_notice_and_buy: settings.nolworld?.auto_notice_and_buy ?? true,
            nol_block_delay_ms: settings.nolworld?.block_delay_ms ?? 600,
            nol_custom_blocks: settings.nolworld?.custom_blocks ?? '',
