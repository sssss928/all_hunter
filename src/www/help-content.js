...
Object.assign(HELP_CONTENT, {
  nol_check_interval: {
    title: 'NOL 監控間隔',
    short: '活動詳情頁檢查 Buy Now 狀態的間隔，建議 5 秒。',
    detail: `
      <p>只用於 NOL 活動詳情頁的可購買狀態檢查；排隊頁與購票頁不會套用一般的「自動刷新頁面間隔」。</p>
      <table class="table table-sm table-bordered">
        <tbody>
          <tr><td><code>3 秒</code></td><td>較積極，請留意網站限制與連線負載。</td></tr>
          <tr><td><code>5 秒</code>（建議）</td><td>反應速度與穩定性較平衡。</td></tr>
          <tr><td><code>8–15 秒</code></td><td>較保守，適合長時間監看。</td></tr>
        </tbody>
      </table>`,
  },
  nol_pre_sale_mode: {
    title: 'NOL 預售模式',
    short: '在開票前的倒數或 Gate 頁面維持原頁，不主動刷新。',
    detail: '<p>適合已進入官方倒數／Gate 的情況。程式會持續檢查按鈕是否可用並在可用時前進，避免刷新後失去目前頁面狀態。</p>',
  },
  nol_lock_only_mode: {
    title: 'NOL 鎖票模式',
    short: '鎖定座位後停止自動往付款完成方向操作，保留瀏覽器給使用者處理。',
    detail: '<p>啟用後，進入訂單／付款階段即停止 NOL 自動化狀態機，但不關閉瀏覽器。付款資料、OTP 或其他敏感步驟仍由使用者完成。</p>',
  },
  nol_auto_notice_and_buy: {
    title: 'NOL 自動進入購票',
    short: '自動關閉活動頁 Notice，並在 Buy Now 可用時立即點擊，包含第一次啟動。',
    detail: '<p>程式會先關閉 Notice，等待遮罩動畫完全消失，再以瀏覽器實體座標點擊 Buy Now；若頁面沒有前進，會短暫重試而不重新整理活動頁。</p>',
  },
  nol_block_delay_ms: {
    title: 'NOL 區塊掃描延遲',
    short: '購票頁每次日期、區域、座位與下一步操作之間的節流時間。',
    detail: `
      <p><code>500–700 ms</code> 通常最穩定，預設 <code>600 ms</code>。降低到 <code>300–450 ms</code> 反應較快，但頁面尚未更新完成時可能造成重複點擊。</p>
      <p class="mb-0 text-muted small">此值不是活動詳情頁刷新間隔。</p>`,
  },
  nol_custom_blocks: {
    title: 'NOL 指定座位區塊',
    short: '優先掃描指定的座位區塊編號或名稱。',
    detail: '<p>可用逗號或分號分隔，例如 <code>104;206;312</code>。留空時沿用「區域關鍵字」。找不到指定區域時是否遞補，取決於「區域自動遞補」。</p>',
  },
  nol_ticket_pool_enabled: {
    title: 'NOL 票池查詢',
    short: '在背景查詢各場次與票種剩餘數量，協助判斷可用票池。',
    detail: '<p>查詢在背景執行，不會阻塞瀏覽器操作。若網站回應不穩或只想使用畫面流程，可關閉以減少額外網路請求。</p>',
  },
  nol_ticket_pool_refresh_minutes: {
    title: 'NOL 票池更新間隔',
    short: '背景票池資料的更新頻率，建議 15 分鐘。',
    detail: '<p>此設定與 Buy Now 監控、座位區塊掃描無關。過短會增加額外請求，通常維持 <code>15</code> 分鐘即可。</p>',
  },
  nol_security_handoff: {
    title: 'NOL 安全驗證接手',
    short: '偵測受保護驗證後保留目前流程，驗證合法完成後自動續跑。',
    detail: `
      <p>程式不會重新整理或丟失已選日期、場次與排隊狀態；驗證畫面消失後自動從原步驟繼續。</p>
      <p><strong>Turnstile、圖片 CAPTCHA、hCaptcha、滑塊或其他官方安全機制不會被繞過。</strong></p>`,
  },
  nol_session_timeout_minutes: {
    title: 'NOL 購票階段逾時',
    short: '購票頁達到時限仍未進入訂單／付款時，自動回到活動頁開始新一輪。',
    detail: '<p>預設 10 分鐘。排隊時間不計入，只有進入日期／座位購票流程後才開始計時。成功進入訂單或付款階段後不會重啟。</p>',
  },
  nolworld_account: {
    title: 'NOL World 自動登入',
    short: '儲存 NOL World Email 與密碼，啟動時自動填入登入表單。',
    detail: `
      <p>若尚未登入，程式會先開啟 NOL World Email 登入頁，填入帳號密碼，並在官方驗證完成後送出。</p>
      <p><strong>安全提醒：</strong>密碼會儲存在本機 <code>settings.json</code>。請勿分享設定檔或發布含有個人資料的壓縮檔。</p>
      <p class="mb-0 text-muted small">Cloudflare Turnstile 需由使用者完成，程式不會繞過網站的互動式安全驗證。</p>`,
  },
});
...
Object.assign(HELP_CONTENT_EN_META, {
  nol_check_interval: {
    title: 'NOL check interval',
    short: 'How often the event detail page checks the Buy Now state. Recommended: 5 seconds.',
    detailHtml: '<p>This setting applies only to the NOL event detail page. Queue and booking pages do not use the general auto-reload interval.</p>',
  },
  nol_pre_sale_mode: {
    title: 'NOL pre-sale mode',
    short: 'Keep the official countdown or gate page open without reloading it.',
    detailHtml: '<p>The bot watches the gate control and advances when it becomes available, preserving the current official page state.</p>',
  },
  nol_lock_only_mode: {
    title: 'NOL lock-only mode',
    short: 'Stop automation after reaching the order or payment stage and leave the browser open.',
    detailHtml: '<p>Use this when you want to complete payment, OTP, and other sensitive steps manually.</p>',
  },
  nol_auto_notice_and_buy: {
    title: 'Automatic Notice / Buy',
    short: 'Close the event Notice and click Buy Now whenever it is enabled, including on first launch.',
    detailHtml: '<p>MyHunter waits for the Notice close animation to finish, uses a browser-level click, and retries briefly if the page does not advance.</p>',
  },
  nol_block_delay_ms: {
    title: 'NOL block scan delay',
    short: 'Delay between date, area, seat, and Next actions. Recommended: 500–700 ms.',
    detailHtml: '<p>The default 600 ms balances speed and page-render stability. This is not the event-page refresh interval.</p>',
  },
  nol_custom_blocks: {
    title: 'NOL custom seat blocks',
    short: 'Prioritize specified block numbers or names.',
    detailHtml: '<p>Separate values with commas or semicolons, for example <code>104;206;312</code>. Leave blank to use area keywords.</p>',
  },
  nol_ticket_pool_enabled: {
    title: 'NOL ticket-pool query',
    short: 'Query remaining ticket grades and performances in the background.',
    detailHtml: '<p>Disable this if you want fewer background requests and only need browser-based selection.</p>',
  },
  nol_ticket_pool_refresh_minutes: {
    title: 'NOL ticket-pool refresh',
    short: 'Background ticket-pool refresh frequency. Recommended: 15 minutes.',
    detailHtml: '<p>This does not control Buy Now monitoring or seat-block scanning.</p>',
  },
  nol_security_handoff: {
    title: 'NOL security handoff',
    short: 'Preserve the active workflow and resume automatically after protected verification is legitimately completed.',
    detailHtml: '<p>The page is not refreshed and selected workflow state is retained. Turnstile, image CAPTCHAs, hCaptcha, sliders, and other official security controls are not bypassed.</p>',
  },
  nol_session_timeout_minutes: {
    title: 'NOL booking timeout',
    short: 'Restart from the event page when the booking flow exceeds its time window.',
    detailHtml: '<p>The timer starts after the queue, when the date/seat booking flow opens. It stops after reaching the order or payment stage.</p>',
  },
  nolworld_account: {
    title: 'NOL World automatic login',
    short: 'Store the NOL World email and password and fill the login form automatically.',
    detailHtml: '<p>The password is stored locally in <code>settings.json</code>. Never share that file. Cloudflare Turnstile must be completed manually.</p>',
  },
});
