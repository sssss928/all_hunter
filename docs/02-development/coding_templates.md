...
async def handle_cloudflare_verification(tab, config_dict):
    """偵測受保護驗證並保留流程，合法完成後自動續跑。"""
    debug = util.create_debug_logger(config_dict)
    while await detect_cloudflare_challenge(tab):
        debug.log("等待官方安全驗證完成；目前頁面與 workflow state 保持不變")
        await asyncio.sleep(1)
    debug.log("安全驗證完成，恢復平台流程")
```
...
**注意事項**：不點擊驗證元件、不注入 token，也不使用外部解題服務。
