...
✅ **NoDriver 完全支援**：TixCraft、Teamear、TicketMaster、Indievox、KKTIX、TicketPlus、iBon、年代售票、寬宏售票、Cityline 買飛、HKTicketing 快達票、KHAM、FamiTicket、FANSI GO、FunOne、NOL World / Global Interpark

### NOL World 執行重點

- 活動頁會先自動關閉官方 `Notice`，等待遮罩動畫結束後再點擊 `Buy Now`，首次啟動也會執行。
- 日期、時間與 `Next` 採分步狀態確認；優先監聽 DOM 變化，時間按鈕尚未渲染時不會提早送出。
- 排隊頁不重新整理；進入購票流程後使用獨立的區塊掃描延遲，避免與活動頁監控間隔互相干擾。
- 登入帳號與密碼可由「自動填表單」頁籤設定；設定範本不會內含任何憑證。
- 官方 Turnstile 或互動式安全驗證會保留目前頁面與流程狀態，使用者完成後自動續跑；程式不偽造 token 或繞過平台安全機制。

### 執行架構與效能

- 12 個 Python 平台模組集中由 hostname 路由並快取判定結果，涵蓋 README 所列的多個網站家族。
- 共用狀態機記錄每次流程轉換，有限退避避免錯誤時高速空轉；單一平台失敗不會直接終止整個主程式。
- DOM 操作優先等待實際狀態變更，固定 sleep 僅作回退，降低 React／SPA 尚未完成渲染造成的卡頓與漏點。
- 結構化日誌會遮蔽密碼、token、cookie、授權標頭與 webhook 等敏感欄位。
