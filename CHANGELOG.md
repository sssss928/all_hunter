...
- NOL World 新增滑動拼圖與 hCaptcha 類型偵測，沿用安全 handoff/resume：保留頁面、排隊與購票狀態，使用者完成後自動續跑
- 參考原 NOL 擴充功能的 `.captchSliderInner` 與保護驗證狀態偵測，但不移植自動拖滑或破解邏輯
- 修正 TixCraft 手動輸入 4 碼驗證碼後仍停在原頁的問題；現在會檢查票數、同意勾選與驗證碼欄位，再點送出或使用 `requestSubmit()`
- TixCraft OCR 自動填入後也優先使用新的可靠送出 helper；若 helper 無法送出，仍保留原本既有送出路徑作為回退
- 保留既有 TixCraft、Ticketmaster、iBon、KHAM、FunOne 等圖片 CAPTCHA/OCR 程式碼，未刪除原有平台處理邏輯

## 1.2.0 - 2026-06-23

- 新增共用工作流程狀態機、有限重試策略、MutationObserver DOM 等待、結構化日誌與敏感資料遮蔽
- 新增以 hostname 為基礎且具快取的平台路由，避免主迴圈反覆比對 URL，並杜絕查詢字串造成誤判
- 全面清理 12 個平台模組與共用程式的裸露例外、錯誤 membership 比較、布林比較及未使用 import
- 強化 NOL World Notice、Buy Now、日期、時間與 Next 流程；改用 DOM 狀態變更優先、短暫輪詢回退
- NOL 排隊與購票 10 分鐘工作階段保留狀態；座位競爭失敗會自動關閉訊息、清除失效選擇並重新掃描
- 官方 Turnstile／互動式安全驗證改為 handoff/resume：保留瀏覽器與監控狀態，使用者完成後自動續跑，不注入或偽造 token
- NOL 圖片文字驗證在正式網站採安全交接；OCR 正規化與共識演算法僅供授權測試圖片及既有支援平台使用
- 設定範本不含帳號、密碼或通知憑證，OCR 自動送出改為預設關閉，並新增 `.env.example`
- CI 擴大正確性規則、全平台 import 檢查；Release 同時輸出 Windows 執行包、完整 source ZIP 與 SHA-256
- 修正 Windows PowerShell source staging 清理規則誤刪全部檔案，並加入必要檔案與檔案數防呆
- 新增核心元件、平台路由、安全交接及 NOL 流程回歸測試，完整測試數提升至 46 項

## 1.1.2 - 2026-06-23

- 修正 NOL World 首次啟動時只記錄 Buy Now 狀態、不會立即購買的問題；舊設定檔也預設啟用完整自動流程
- Notice 關閉與 Buy Now 點擊改為兩階段確認，等待官方遮罩動畫消失後再以瀏覽器座標點擊
- Buy Now 若被 React 更新或殘留遮罩吃掉，最多快速重試 10 次後短暫退避，不重新整理活動頁
- 日期辨識支援數字與英文月份、`data-date`／ARIA 標籤；日期與時間分步完成後才尋找 Next
- 新增 NOL 日期、場次與 Next 的 DOM 狀態回歸測試，CI 與 Release 同步檢查設定頁 JavaScript
- 依目前官方路徑更新 NOL 登入網址與 `/en/login` 狀態辨識

## 1.1.1 - 2026-06-23

- NOL World「自動填表單」新增 Email、密碼與自動登入；官方 Turnstile 完成後自動送出
- 修正選擇日期／時間後未自動 Next：新版 onestop 與舊版 Global Interpark 改為分步狀態機
- 強化 NOL 文字圖片驗證碼偵測、React 欄位填值與自動送出，並明確保留互動式安全驗證給使用者完成
- 新增原生 alert、DOM 對話框與座位已被選擇的恢復流程，關閉訊息後清除失效座位並重新掃描
- 新增購票階段 10 分鐘逾時重啟，排隊時間不計入，逾時後回活動頁開始下一輪
- NOL World 隱藏通用「自動刷新頁面間隔」，只使用 NOL 監控間隔與區塊掃描延遲
- NOL 設定與自動登入欄位新增中英文右側說明面板
- 確認 NOL World 執行流程不含激活碼、裝置綁定或私人授權伺服器

## 1.1.0 - 2026-06-22

- 新增 NOL World / Global Interpark 平台模組，將 NOL Ticket Monitor 的監控、票池、預售倒數、排隊、場次、座位與文字驗證碼流程移植至 Python / nodriver
- NOL 排隊頁採狀態輪詢且禁止自動刷新；詳情頁、預售 Gate 與購票頁分別使用獨立節流頻率，避免主迴圈重複操作造成卡頓
- NOL 票池同時查詢 10 組 PlaySeq，並以有限執行緒並行縮短查詢時間，不阻塞瀏覽器事件迴圈
- 設定介面新增 NOL World 快速入口、預售模式、鎖票模式、初始購買、區塊掃描、票池與 OCR 選項
- 原擴充功能中的硬編碼 Telegram 憑證未移植；NOL 通知安全地沿用 MyHunter 的 Discord / Telegram 使用者設定
- CI、PyInstaller hidden imports、靜態契約與發布文件同步加入 `platforms.nolworld`

- 恢復 TixCraft / Ticketmaster 被截斷的完整主流程與 18 個缺失函式
- 修復 FunOne、HKTicketing、KHAM 與共用工具中的未定義名稱及無效跳脫字元
- 補回 KKTIX 暫停期間登入處理，並加入售罄／競爭失敗彈窗的一次性偵測與重載
- 縮短 KKTIX DOM 元素快取時間，降低 SPA 更新後沿用失效元素造成卡頓的風險
- 新增 Python 編譯、Ruff 正確性檢查、靜態契約測試與應用程式匯入測試
- 更新 GitHub Actions CI 與語意化版本 Release 流程，Release 同時產生 ZIP 與 SHA-256
- Release 改為在 Windows runner 以 PyInstaller 建置 `nodriver_tixcraft.exe` 與 `settings.exe`，輸出可直接執行的完整發布包
- 修復 PyInstaller 發布包中 `settings.exe` 錯誤從 `_internal/www` 尋找設定頁，造成 `/settings.html` 與 favicon 回傳 404
